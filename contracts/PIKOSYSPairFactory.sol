...
import {PIKOSYSPairCloner} from "./lib/PIKOSYSPairCloner.sol";
import {IPIKOSYSPairFactoryLike} from "./IPIKOSYSPairFactoryLike.sol";
import {PIKOSYSPairEnumerableETH} from "./PIKOSYSPairEnumerableETH.sol";
import {PIKOSYSPairEnumerableERC20} from "./PIKOSYSPairEnumerableERC20.sol";
import {PIKOSYSPairMissingEnumerableETH} from "./PIKOSYSPairMissingEnumerableETH.sol";
import {PIKOSYSPairMissingEnumerableERC20} from "./PIKOSYSPairMissingEnumerableERC20.sol";

contract PIKOSYSPairFactory is Ownable, IPIKOSYSPairFactoryLike {
    using PIKOSYSPairCloner for address;
    ...
    PIKOSYSPairEnumerableETH public immutable enumerableETHTemplate;
    PIKOSYSPairMissingEnumerableETH public immutable missingEnumerableETHTemplate;
    PIKOSYSPairEnumerableERC20 public immutable enumerableERC20Template;
    PIKOSYSPairMissingEnumerableERC20
    ...
    mapping(PIKOSYSRouter => RouterStatus) public override routerStatus;
    ...
    event RouterStatusUpdate(PIKOSYSRouter router, bool isAllowed);

    constructor(
        PIKOSYSPairEnumerableETH _enumerableETHTemplate,
        PIKOSYSPairMissingEnumerableETH _missingEnumerableETHTemplate,
        PIKOSYSPairEnumerableERC20 _enumerableERC20Template,
        PIKOSYSPairMissingEnumerableERC20 _missingEnumerableERC20Template,
    ...
    function createPairETH(
        ...
        PIKOSYSPair.PoolType _poolType,
        uint128 _delta,
        uint96 _fee,
        uint128 _spotPrice,
        uint256[] calldata _initialNFTIDs
    ) external payable returns (PIKOSYSPairETH pair) {
        require(
    ...
    /**
        @notice Checks if an address is a PIKOSYSPair. Uses the fact that the pairs are EIP-1167 minimal proxies.
    ...
    returns (bool)
    {
        if (variant == PairVariant.ENUMERABLE_ERC20) {
            return
            PIKOSYSPairCloner.isERC20PairClone(
                address(this),
            ...
            return
            PIKOSYSPairCloner.isETHPairClone(
                address(this),
    ...
    onlyOwner
    {
        ...
        if (isAllowed) {
            require(
                !routerStatus[PIKOSYSRouter(target)].wasEverAllowed,
                "Can't call router"
            );
        }
    ...
    /**
        ...
        @param isAllowed True to whitelist, false to remove from whitelist
     */
    function setRouterAllowed(PIKOSYSRouter _router, bool isAllowed)
    external
    onlyOwner
    {
    ...
    */
    ...
    function depositERC20(
        ...
        if (
            ...
            if (token == PIKOSYSPairERC20(recipient).token()) {

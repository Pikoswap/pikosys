...
abstract contract PIKOSYSPair is
...
ERC1155Holder
{
    ...
    // 90%, must <= 1 - MAX_PROTOCOL_FEE (set in PIKOSYSPairFactory)
    ...
    /**
        ...
        @param isRouter True if calling from PIKOSYSRouter, false otherwise. Not used for
        ETH pairs.
    ...
    function swapTokenForAnyNFTs(
        ...
        IPIKOSYSPairFactoryLike _factory = factory();
        ICurve _bondingCurve = bondingCurve();
        IERC721 _nft = nft();
    ...
    function factory() public pure returns (IPIKOSYSPairFactoryLike _factory) {
    ...
    /**
        ...
        @param isRouter Whether or not the caller is PIKOSYSRouter
        @param routerCaller If called from PIKOSYSRouter, store the original caller
        @param _factory The PIKOSYSPairFactory which stores PIKOSYSRouter allowlist info
    ...
    /**
        ...
        @dev We send ETH back to the caller even when called from PIKOSYSRouter because we do an aggregate slippage check for certain bulk swaps. (Instead of sending directly back to the router caller)
        Excess ETH sent for one swap can then be used to help pay for the next swap.
     */
    function _refundTokenToSender(uint256 inputAmount) internal virtual;

    /**
        @notice Sends protocol fee (if it exists) back to the PIKOSYSPairFactory from the pair
     */
    ...
    /**
        ...
        @dev This is used by the PIKOSYSPair's swapNFTForToken function.
    ...
    /**
        @dev Used internally to grab pair parameters from calldata, see PIKOSYSPairCloner for technical details
     */
